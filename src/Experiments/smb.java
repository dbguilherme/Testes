	  ...
	  AbstractBlockingMethod blocking;
		...
		if(profilesPathB != null){
					...
					blocking = new TokenBlocking(new List[]{profiles[0], profiles[1]});
					 ...
					 System.out.println("  block size A "+ blocks.size());
					 AbstractEfficiencyMethod blockPurging = new ComparisonsBasedBlockPurging(1.0001);
						blockPurging.applyProcessing(blocks);
//					
						System.out.println("  block size B "+ blocks.size());
					 BlockFiltering bf = new BlockFiltering(0.8);
					    bf.applyProcessing(blocks);	
					    System.out.println("  block size C "+ blocks.size());
		}else	{
			profiles= new List[1];
			profiles[0] = (List<EntityProfile>) SerializationUtilities.loadSerializedObject(profilesPathA);
...
//		}
		...
		//for (int i = 1; i <= 2;i++)
		{
			...
			//while(tamanho<=1000)
			{
				...
				for (j = 0;j< 5; j++) 
				{
					swep.applyProcessing(j, classifiers, ebc);
